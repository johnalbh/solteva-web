...
export async function generateMetadata({ params }: Props) {
  ...
  return generateSEOMeta({
    ...
    locale,
    path: `/productos/plataformas/${slug}`,
...
export default async function ProductDetailPage({ params }: Props) {
  const { locale, slug } = await params;
  const product = getProductBySlug(slug);
  if (!product || product.category !== "platforms") notFound();
  ...
  return (
    <ProductDetailLayout
      product={product}
      categoryProducts={getProductsByCategory("platforms")}
      locale={locale}
      categorySlugEs="plataformas"
      categoryLabel={locale === "es" ? "Plataformas Elevadoras" : "Lifting Platforms"}
      categoryHref="/productos/plataformas"
      ...
      jsonLd={generateProductJsonLd({ ...product, category: "plataformas" }, locale)}
    />
  );
}
