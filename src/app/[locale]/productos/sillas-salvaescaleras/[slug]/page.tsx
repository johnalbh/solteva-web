...
export async function generateStaticParams() {
  ...
  return products
    .filter((p) => p.category === "stairlifts")
...
export async function generateMetadata({ params }: Props) {
  ...
  return generateSEOMeta({
    ...
    locale,
    path: `/productos/sillas-salvaescaleras/${slug}`,
...
export default async function ProductDetailPage({ params }: Props) {
  const { locale, slug } = await params;
  const product = getProductBySlug(slug);
  if (!product || product.category !== "stairlifts") notFound();
  ...
  return (
    <ProductDetailLayout
      product={product}
      categoryProducts={getProductsByCategory("stairlifts")}
      locale={locale}
      categorySlugEs="sillas-salvaescaleras"
      categoryLabel={locale === "es" ? "Sillas Salvaescaleras" : "Stairlifts"}
      categoryHref="/productos/sillas-salvaescaleras"
      ...
      jsonLd={generateProductJsonLd({ ...product, category: "sillas-salvaescaleras" }, locale)}
    />
  );
}
